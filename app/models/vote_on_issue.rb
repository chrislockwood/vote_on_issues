class VoteOnIssue < ActiveRecord::Base
  ...
  def self.list_of_up_voters_for_issue(issue_id)
    ...
    find_by_sql( ["SELECT `vote_on_issues`.`vote_value` AS vote_value, `users`.`login` AS user_login FROM `vote_on_issues` LEFT JOIN `users` ON (`users`.`id` = `vote_on_issues`.`user_id`) WHERE (`issue_id` = ? AND `vote_value` > 0) ORDER BY user_login ASC", issue_id] )
  end
